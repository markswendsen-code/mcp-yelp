...
export class YelpSession {
    ...
    async searchBusinesses(options) {
        try {
            ...
            if (options.category)
                url += `&cflt=${encodeURIComponent(options.category)}`;
            await this.browser.navigate(url);
            await this.browser.waitForSelector('[data-testid="serp-ia-card"]');
    ...
    async getReviews(businessId, options) {
        try {
            ...
            const reviews = await this.browser.evaluateWithArg((maxReviews) => {
    ...
    async getPhotos(businessId, options) {
        try {
            let url = `https://www.yelp.com/biz_photos/${businessId}`;
            if (options?.category && options.category !== 'all')
                url += `?tab=${options.category}`;
            await this.browser.navigate(url);
            await this.browser.waitForSelector('[class*="photo-box"]');
            const limit = options?.limit || 10;
            const photos = await this.browser.evaluateWithArg((maxPhotos) => {
    ...
    async findSimilar(businessId, limit = 5) {
        try {
            await this.browser.navigate(`https://www.yelp.com/biz/${businessId}`);
            await this.browser.waitForSelector('[class*="similar"], [class*="youMightAlsoConsider"]');
            const similar = await this.browser.evaluateWithArg((maxSimilar) => {
    ...
    async getTrending(location, category) {
        try {
    ...
    async getCollections(location, category) {
        try {
    ...
    async checkWaitTime(businessId) {
        try {
            await this.browser.navigate(`https://www.yelp.com/biz/${businessId}`);
    ...
    async getDeals(location, category) {
        try {
